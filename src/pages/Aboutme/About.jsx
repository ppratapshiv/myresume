...
function About({title, subtitle, titleContent}){
 return (
    <>
      <LeftSidePane name='Guddu Pandit' designation='King of Mirzapur' profilepic={profilepic}/>
      <div className="px-2 pt-[5rem] pb-10 overflow-auto w-[80%] overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-[#ccc] dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500">
        <div className="flex items-center">
            <div className='w-[40%] flex flex-col gap-3'>
              <span className='text-2xl font-medium'>{title='HI, I\'M A Frontend'}</span>
              <h1 className='text-8xl font-bold'>{subtitle='Developer'}</h1>
            ...
            <div className="ms-auto w-[50%]">
              <div className="relative w-max ms-auto overflow-hidden">
                <img
                  ...
                  className="block"
                />
                <div className="absolute inset-0 bg-[#5271ff] opacity-50 z-[-1] p-2 animate-tilt"></div>
                </div>
            </div>
        </div>
        ...
        <div className="flex items-end">
          ...
          <div className="w-[20%]">
            <Link to='/service-and-pricing'><Button icon={<IoMdArrowForward />} label='Service & Pricing' color='bg-[#5271ff] text-white hover:bg-black hover:text-white w-max ms-auto'/></Link>
          </div>
        </div>
        ...
        <div className={`flex flex-wrap gap-3 my-5`}>
            <ContactCta bgImage={bgImage}>
             <div className="flex items-center">
                ...
                <div className="w-[50%]">
                  <Link to="/contact">
                    <Button icon={<IoMdArrowForward />} label={"Let's Get In Touch"} color={'bg-[#5271ff] text-white hover:bg-white hover:text-black w-max ms-auto'}/>
                  </Link>
                </div>
             </div>
            </ContactCta>
        ...
        <div className="flex items-end">
          <div className="w-[80%]">
            <SectionTitle secTitle='Featured Projects' secSubTitle="My step-by-step guide ensures a smooth project journey, from the initial consultation to the final delivery. I take care of every detail, allowing you to focus on what you do best." exClassName={'border-l-4 border-[#5271ff] pl-4'}/>
          </div>
          <div className="w-[20%]">
            <Link to='/portfolio'><Button icon={<IoMdArrowForward />} label='View Portfolio' color='bg-[#5271ff] text-white hover:bg-black hover:text-white w-max ms-auto'/></Link>
          </div>
        </div>
        ...
        <div className="flex items-end">
          <div className="w-[80%]">
            <SectionTitle secTitle='Latest Blog Posts' secSubTitle="More than 1500+ agencies using Portfolify" exClassName={'border-l-4 border-[#5271ff] pl-4'}/>
          </div>
          <div className="w-[20%]">
            <Link to='/blog'>
              <Button icon={''} label='See All Articles' color='bg-[#5271ff] text-white hover:bg-black hover:text-white w-max ms-auto'/>
            </Link>
          </div>
        </div>
