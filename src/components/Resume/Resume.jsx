      ...
      <div className="p-10 bg-[#fafafa] rounded-[25px]">
        <div className="flex justify-between">
            <div className="flex flex-col gap-2">
              <h1 className="text-6xl text-[#5271ff] font-bold">{user.Name}</h1>
        ...
        <div className="flex justify-between items-center">
          <div className="w-[180px] h-[180px] bg-[#5271ff] rounded-full overflow-hidden">
          ...
          <div className="w-[70%]">
                {user.breifIntro.map((breif, index)=>(
                  <div className="flex flex-col gap-4" key={index}>
                     <h2 className="text-3xl font-bold relative after:absolute leading-[1.2] border-[#5271ff] border-l-[5px] ps-3 text-[#5271ff]">{breif.title ? breif.title : ''}</h2>
                     <div className="flex justify-between">
                        <h4 className="text-lg font-medium">{breif.subtitle ? breif.subtitle:''}</h4>
                        <p className="flex">{breif.company ? breif.company:''} {breif.timeLine ? breif.timeLine : ''}</p>
                     </div>
                     <p>{breif.summary}</p>
          ...
          <div className="w-[25%]">
            <div className="ps-2 flex flex-col gap-4 border-l-[1px] border-[#000]">
              {user.sidePoints.map((sidePoint, index)=>(
                <div className="flex flex-col gap-4" key={index}>
                  <h2 className="text-3xl font-bold relative after:absolute leading-[1.2] border-[#5271ff] border-l-[5px] ps-3 text-[#5271ff]">     {sidePoint.title ? sidePoint.title:''}
                  </h2>
                  <h4 className="text-lg font-medium">{sidePoint.subtTitle ? sidePoint.subtTitle:''}</h4>
